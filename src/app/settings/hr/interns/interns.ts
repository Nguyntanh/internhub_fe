...
// User INTERN chưa có hồ sơ
interface AvailableUser { id: number; name: string; email: string; phone: string; departmentName?: string; }
...
export class HrInternsComponent implements OnInit {
  ...
  private API_AVAILABLE = 'http://localhost:8090/api/user/interns/available';
  ...
  // Danh sách user INTERN chưa có hồ sơ
  ...
  // userId được chọn từ dropdown (chỉ dùng khi thêm mới)
  ...
  loadMentors()      { this.http.get<Mentor[]>(this.API_MENTOR).subscribe(r => { this.MENTORS = r; this.cdr.detectChanges(); }); }
  loadManagers()     { this.http.get<Manager[]>(this.API_MANAGER).subscribe(r => { this.MANAGERS = r; this.cdr.detectChanges(); }); }
  loadUniversities() { this.http.get<University[]>(this.API_UNIV).subscribe(r => { this.UNIVERSITIES = r; this.cdr.detectChanges(); }); }
  loadDepartments()  { this.http.get<Department[]>(this.API_DEPT).subscribe(r => { this.DEPARTMENTS = r; this.cdr.detectChanges(); }); }

  loadAvailableUsers() {
    this.http.get<AvailableUser[]>(this.API_AVAILABLE).subscribe({
      next: r => { this.availableUsers = r; this.cdr.detectChanges(); },
      error: () => {}
    });
  }
  ...
  loadPositions() {
    ...
    this.http.get<any[]>(this.API_POS).subscribe(res => {
      this.ALL_POSITIONS = res.map(p => ({ id: p.id, name: p.name, departmentId: p.departmentId }));
      res.forEach(p => {
  ...
  loadInterns(): void {
    this.http.get<any[]>(this.API).subscribe(res => {
      this.interns = res.map(i => ({
        id: i.id, fullName: i.fullName, email: i.email, phone: i.phone,
        major: i.major,
        universityId: i.universityId, universityName: i.universityName,
        deptId: i.departmentId, deptName: i.departmentName,
        positionId: i.positionId, posName: i.positionName,
        status: i.status,
        startDate: i.startDate, endDate: i.endDate,
        mentorId: i.mentorId, managerId: i.managerId
      }));
      this.applyFilters();
      this.cdr.detectChanges();
    });
  ...
  buildForm(): void {
    this.internForm = this.fb.group({
      fullName:     ['', [Validators.required, Validators.minLength(2), Validators.pattern(/^[a-zA-ZÀ-ỹà-ỹĂăÂâĐđÊêÔôƠơƯư\s]+$/)]],
  ...
  // Khi chọn user từ dropdown → pre-fill form, lưu userId
  onSelectExistingUser(event: Event) {
    const val = (event.target as HTMLSelectElement).value;
    if (!val) {
      this.selectedUserId = null;
      this.internForm.patchValue({ fullName: '', email: '', phone: '' });
      return;
    }
  ...
  onDeptFilterChange(): void {
    ...
    this.filterPositionOptions = dept ? (this.DEPT_POSITIONS[dept.id] || []).map(p => p.name) : [];
    this.applyFilters();
    this.cdr.detectChanges();
  }
  ...
  openEditModal(intern: Intern): void {
    ...
    this.internForm.patchValue({
      fullName: intern.fullName, email: intern.email, phone: intern.phone,
      major: intern.major, universityId: intern.universityId,
      startDate: intern.startDate, endDate: intern.endDate,
      mentorId: intern.mentorId, managerId: intern.managerId,
  ...
  saveIntern(): void {
    ...
    // Nếu chọn từ danh sách → đính kèm userId vào payload
    const payload = this.selectedUserId ? { ...val, userId: this.selectedUserId } : val;
    ...
    if (this.isEditMode && this.editingId) {
      ...
      this.http.post(this.API, payload).subscribe({
        next: () => {
          ...
          this.loadInterns();
          this.loadAvailableUsers(); // Làm mới danh sách sau khi tạo
          this.closeFormModal();
        },
        error: (err) => {
          if (err.status === 409) {
            this.duplicateError = this.selectedUserId
              ? 'Người dùng này đã có hồ sơ thực tập.'
              : `Email "${val.email}" đã tồn tại trong hệ thống.`;
          } else {
            this.showToastMessage('Lỗi khi tạo hồ sơ. Vui lòng thử lại.');
          }
        }
      });
    }
  ...
  openDeleteModal(intern: Intern): void { this.deletingIntern = intern; this.isDeleteModalOpen = true; }
  closeDeleteModal(): void { this.isDeleteModalOpen = false; this.deletingIntern = null; }
  ...
  openStatusModal(intern: Intern): void { this.statusIntern = intern; this.selectedStatus = intern.status; this.isStatusModalOpen = true; }
  closeStatusModal(): void { this.isStatusModalOpen = false; this.statusIntern = null; }

  saveStatus(): void {
    if (!this.statusIntern) return;
    this.http.patch(`${this.API}/${this.statusIntern.id}/status`, { status: this.selectedStatus }).subscribe(() => {
  ...
  triggerImport(): void { document.getElementById('xlsx-input')?.click(); }
  ...
  handleImport(event: Event): void {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        const jsonData: any[] = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '', raw: false } as any);
        ...
        if (errors.length > 0) { this.showToastMessage(`Có ${errors.length} dòng lỗi: ${errors.join(', ')}`); return; }
        this.importInternsSequentially(internsToImport.map(i => i.data));
      } catch (err) { this.showToastMessage('File Excel không hợp lệ hoặc bị hỏng!'); }
    };
  ...
  private mapRowToIntern(row: any[], headers: string[], rowIndex: number): { data?: any; error?: string } {
    ...
    if (!fullName || !email || !phone) return { error: `Dòng ${rowIndex + 2}: Thiếu họ tên / email / SĐT` };
    const universityObj = university ? this.UNIVERSITIES.find(u => u.name.toLowerCase().includes(university.toLowerCase())) : null;
    const universityId = universityObj?.id ?? null;
    let positionId: number | null = null;
    let departmentId: number | null = null;
    if (position) {
      const posNorm = position.toLowerCase().replace(/\s+/g, ' ').trim();
      const positionObj = this.ALL_POSITIONS.find(p => p.name.toLowerCase().replace(/\s+/g, ' ').trim() === posNorm || p.name.toLowerCase().includes(posNorm));
      if (positionObj) { positionId = positionObj.id; departmentId = positionObj.departmentId; }
    }
    if (department && !departmentId) {
      const deptNorm = department.toLowerCase().replace(/\s+/g, ' ').trim();
      const deptObj = this.DEPARTMENTS.find(d => d.name.toLowerCase().replace(/\s+/g, ' ').trim() === deptNorm || d.name.toLowerCase().includes(deptNorm));
      if (deptObj) departmentId = deptObj.id;
    }
    const formatDate = (val: any): string | null => {
      if (!val) return null;
      if (typeof val === 'number') return new Date(Math.floor(val - 25569) * 86400 * 1000).toISOString().split('T')[0];
    ...
    return { data: { fullName, email, phone: phone.replace(/\D/g, ''), major, universityId, departmentId, positionId, startDate: formatDate(startDate), endDate: end, status: resolvedStatus, mentorId: null, managerId: null } };
  }
  ...
  private async importInternsSequentially(interns: any[]) {
    ...
    for (const intern of interns) {
      try {
        if (this.interns.find(i => i.email === intern.email)) { skipCount++; continue; }
        await this.http.post(this.API, intern).toPromise();
        successCount++;
      } catch (err: any) {
        if (err?.status === 409) skipCount++; else errorCount++;
      }
    }
  ...
  getDeptName(deptId: string): string { return this.DEPARTMENTS.find(d => String(d.id) === deptId)?.name || '—'; }
  getUnivShort(univId: string): string { return this.UNIVERSITIES.find(u => String(u.id) === univId)?.short || '—'; }
  getUnivFull(univId: number): string { return this.UNIVERSITIES.find(u => u.id === univId)?.name || ''; }
  avatarColor(name: string): string {
    const c = ['#3b82f6','#8b5cf6','#ec4899','#f59e0b','#10b981','#06b6d4','#6366f1','#f97316'];
    return c[name.charCodeAt(0) % c.length];
  }
  isInvalid(field: string): boolean { const ctrl = this.internForm.get(field); return !!(ctrl?.invalid && ctrl.touched); }
  ...
  showToastMessage(msg: string): void {
    ...
    this.toastMessage = msg; this.showToastFlag = true;
    this.toastTimer = setTimeout(() => (this.showToastFlag = false), 3000);
  }
  trackById(_: number, item: Intern): number { return item.id; }
