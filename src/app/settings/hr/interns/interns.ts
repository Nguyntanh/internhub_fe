...
export class HrInternsComponent implements OnInit {
  ...
  saveIntern(): void {
    ...
    if (dupEmail) {
      this.duplicateError = `Email "${val.email}" đã tồn tại trong hệ thống.`;
      return;
    }
  ...
  private mapRowToIntern(
      ...
      ? this.UNIVERSITIES.find(
          (u) =>
            u.name.toLowerCase().replace(/\s+/g, ' ') ===
              university.toLowerCase().replace(/\s+/g, ' ') ||
    ...
    if (position) {
      ...
      const positionObj = this.ALL_POSITIONS.find(
        (p) =>
      ...
      if (positionObj) {
        ...
        console.log(
          'Positions có sẵn:',
          this.ALL_POSITIONS.map((p) => p.name),
        );
      }
    }
    ...
    if (department) {
      ...
      if (deptObj) {
        if (departmentId && departmentId !== deptObj.id) {
          console.warn(
            `Dòng ${rowIndex + 2}: Dept từ Excel (${deptObj.name}) khác dept của position (id:${departmentId}), dùng dept của position`,
          );
        }
