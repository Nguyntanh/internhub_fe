...
export class HrInternsComponent implements OnInit {
  ...
  get filteredAvailableUsers(): UserItem[] {
    ...
    return this.availableInternUsers.filter(u =>
  ...
  loadMentors()  { this.http.get<Mentor[]>(this.API_MENTOR).subscribe(res => this.MENTORS = res); }
  loadManagers() { this.http.get<Manager[]>(this.API_MANAGER).subscribe(res => this.MANAGERS = res); }
  ...
  loadAvailableInternUsers(): void {
    this.http.get<any[]>(this.API_USERS).subscribe({
      next: (allUsers) => {
        ...
        this.availableInternUsers = internUsers.filter(u =>
  ...
  saveIntern(): void {
    ...
    if (this.isEditMode && this.editingId) {
      ...
      if (dupEmail) { this.duplicateError = `Email "${val.email}" đã tồn tại.`; return; }
  ...
  handleImport(event: Event): void {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        if (errors.length > 0) { this.showToastMessage(`Có ${errors.length} dòng lỗi.`, true); return; }
        this.importInternsSequentially(internsToImport.map(i => i.data));
      } catch { this.showToastMessage('File Excel không hợp lệ!', true); }
    };
  ...
  private mapRowToIntern(row: any[], headers: string[], rowIndex: number): { data?: any; error?: string } {
    ...
    const universityObj  = this.UNIVERSITIES.find(u => u.name.toLowerCase().includes(university.toLowerCase()));
    let positionId: number | null = null;
    let departmentId: number | null = null;

    if (position) {
      const pos = this.ALL_POSITIONS.find(p => p.name.toLowerCase().includes(position.toLowerCase()));
      if (pos) { positionId = pos.id; departmentId = pos.departmentId; }
    }
    if (department && !departmentId) {
      const dept = this.DEPARTMENTS.find(d => d.name.toLowerCase().includes(department.toLowerCase()));
    ...
    return {
      data: {
        fullName, email, phone: phone.replace(/\D/g, ''), major,
        universityId: universityObj?.id ?? null,
        departmentId, positionId,
        startDate: formatDate(startDate), endDate: end,
        status: resolvedStatus, mentorId: null, managerId: null,
      }
    };
  }
  ...
  private async importInternsSequentially(interns: any[]) {
    ...
    for (const intern of interns) {
      try {
        ...
        if (existing) { skip++; } else { await this.http.post(this.API, intern).toPromise(); ok++; }
      } catch (e: any) { if (e?.status === 409) skip++; else err++; }
    }
