...
| **Thiết lập dữ liệu ban đầu** | **Không áp dụng (Backend)** | Đây là tính năng khởi tạo của Backend. |
...
| **Ghi nhật ký kiểm toán (Audit Log)** | **Chưa hoàn thiện (API & UI)** | Backend có `AuditLogService` nhưng không có service hay UI nào ở Frontend để xem, lọc hoặc quản lý nhật ký kiểm toán. |
| **Quản lý CRUD hạn chế (Department, Role, Function)** | **Chưa hoàn thiện (API & UI)** | Backend có các thực thể này nhưng Frontend chưa có UI hay service rõ ràng để quản lý CRUD. (Ví dụ: Không tìm thấy form quản lý phòng ban). |
...
| **Logic nghiệp vụ cốt lõi cho Thực tập (InternshipPosition, InternshipProfile, MicroTask, Skill, University, TaskSkillRating)** | **Chỉ là khung/Placeholder (UI)** | Các component như `EvaluationSkillsComponent`, `HrInternsComponent`, `PartnersListComponent` chỉ chứa template giữ chỗ (`<p>... works!</p>`), chưa có UI hoặc logic nghiệp vụ thực tế. Frontend **thiếu UI/services** để quản lý các thực thể này. |
...
*   **Quản lý vị trí thực tập (InternshipPosition):** Không có UI/service tương ứng (mặc dù có `partners/list/list.ts` nhưng là placeholder).
*   **Quản lý hồ sơ thực tập sinh (InternshipProfile):** Không có UI/service tương ứng (mặc dù có `hr/interns/interns.ts` nhưng là placeholder).
